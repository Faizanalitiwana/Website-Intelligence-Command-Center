...
export default function HighIssuesPage() {
  ...
  const issues = data?.issues.filter((issue) => issue.severity === "high") ?? [];
  ...
  return (
    <main className="shell">
      <section className="hero">
        ...
        <h1>High Issues</h1>
        <p className="subtitle">High-severity findings from the latest direct website crawl.</p>
      </section>

      <section className="panel">
        <div className="panel-header">
          <div><h2>High Issues</h2><p>{data?.finalUrl ?? "No saved audit"}</p></div>
          <span className="verified">{issues.length} findings</span>
        </div>

        {!data ? (
          <div className="error-box"><strong>No audit result available</strong><p>Run a website audit from the home page first.</p></div>
        ) : issues.length === 0 ? (
          <div className="success-box">No high issues were detected in this audit.</div>
        ) : (
          <div className="issues-list">
            {issues.map((issue, index) => (
              <article className="issue" key={`${issue.code}-${issue.url}-${index}`}>
                <span className="severity high">high</span>
                <div className="issue-content"><strong>{issue.title}</strong><p>{issue.detail}</p><code>{issue.url}</code></div>
              </article>
            ))}
          </div>
        )}
